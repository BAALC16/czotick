(function () {
    ...
    function initActiveMenu() {
        ...
        var currentPath = location.pathname == "/" ? "index" : location.pathname.substring(1);
        if (location.search != "") {
            currentPath = currentPath + location.search;
        }
        var separator = "backend/";
        currentPath = currentPath.substring(currentPath.indexOf(separator) + separator.length);
        if (currentPath) {
            // navbar-nav
            var a = document.getElementById("navbar-nav").querySelector('[nav="' + currentPath + '"]');

            if (a == null) {
                currentPath = currentPath.substring(0, currentPath.indexOf('/'));
                a = document.getElementById("navbar-nav").querySelector('[nav="' + currentPath + '"]');
            }
    ...
    function handleNotification() {
        $("#page-header-notifications-dropdown").click(function (e) {
            e.preventDefault();
            let f = $(this);

            $("#page-header-notifications-count").html('');

            $.ajax({
                url: f.attr("notif-action"),
                dataType: "JSON"
                });
        });
    }
