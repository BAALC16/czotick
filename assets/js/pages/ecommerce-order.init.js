...
var orderList = new List("orderList", options).on("updated", function (list) {
  ...
  var isLast = list.i > list.matchingItems.length - list.page; // make the Prev and Nex buttons disabled on first and last pages accordingly
...
isCount = new DOMParser().parseFromString(orderList.items.slice(-1)[0]._values.id, "text/html");
...
var table = document.getElementById("orderTable"); // save all tr
...
addBtn.addEventListener("click", function (e) {
  if (customerNameField.value !== "" && productNameField.value !== "" && dateField.value !== "" && amountField.value !== "" && paymentField.value !== "") {
    orderList.add({
...
function isStatus(val) {
  switch (val) {
    ...
    case "Returns":
      return '<span class="badge badge-soft-primary text-uppercase">' + val + "</span>";
