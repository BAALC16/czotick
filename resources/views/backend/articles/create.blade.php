...
@endslot
@endcomponent
<form method="POST" id="form" @if ($edit) action="{{ route('articles.update', $article) }}" @else
    action="{{ route('articles.store') }}" @endif enctype="multipart/form-data" {{-- class="form-xhr" --}}>
    @csrf
    ...
    <div class="row  justify-content-center">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    <div class="input-group mb-3">
                        <label class="input-group-text" for="project-title-input">Titre de l'article</label>
                    ...
                    @can('viewAny', App\Models\User::class)
                        <div class="mb-3">
                            <input class="form-check-input" type="checkbox" @if (!$edit || ($edit && $article->actif)) checked @endif  name="actif" value="1" id="defaultCheck1" />
                            <label class="form-check-label" for="defaultCheck1"> Publier</label>
                        </div>
                    @endcan
                    <div class="mb-3">
                        <label class="form-label" for="">Description</label>
                        <textarea id="description" name="content" rows="8">@if ($edit) {!! $article->content !!}@endif </textarea>
                    </div>
                </div>
                <!-- end card body -->
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <input type="file" id="article" name="image_file" accept="image/*" class="filepond" data-show-loader="true"
                        data-allowed-file-extensions="png jpg jpeg gif svg" data-max-file-size="60MB" />
                    @if ($edit)
                        <small>Acienne image</small>
                        <div class="col-4">
                            <a><img src="/public{{ $article->image }}" alt="image" style="width:100%;">
                            </a>
                        </div>
                    @endif
...
@if($edit)
    <script>
        var article={!! json_encode($articleImage) !!};
    </script>
