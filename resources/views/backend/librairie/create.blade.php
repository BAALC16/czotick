...
@endslot
@endcomponent
<form method="POST" id="form" @if ($edit) action="{{ route('library.update', $library) }}" @else
    action="{{ route('library.store') }}" @endif enctype="multipart/form-data" class="form-xhr">
    @csrf
    ...
    <div class="row  justify-content-center">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    <div class="input-group mb-3">
                        <label class="input-group-text" for="project-title-input">Titre </label>
                        <input type="text" class="form-control" id="project-title-input" name="title" @if($edit)
                            value="{{ $library->title }}" @endif />
                    </div>
                </div>
                <!-- end card body -->
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <input type="file" name="document" id="document" class="filepond" data-show-loader="true" accept="application/pdf" data-allowed-file-extensions="pdf"/>
                    @if ($edit)
...
@if($edit)
<script>
    var library={!! json_encode($libraryDoc) !!};
</script>
<script src="{{ URL::asset('assets/js/pages/library_form_edit.js') }}"></script>
@else
    <script src="{{ URL::asset('assets/js/pages/library_form.js') }}"></script>
@endif
