...
@endslot
@endcomponent
<form method="POST" id="form" @if ($edit) action="{{ route('patners.update', $patner) }}" @else
    action="{{ route('patners.store') }}" @endif enctype="multipart/form-data" class="form-xhr">
    @csrf
    ...
    <div class="row  justify-content-center">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    <div class="input-group mb-3">
                        ...
                        <input type="text" class="form-control" id="project-title-input" name="name" @if($edit)
                            value="{{ $patner->name }}" @endif @if ($edit) readonly @endif />
                    </div>
                    <div class="mb-3">
                        <label class="form-label" for="">Description</label>
                        <textarea id="description" name="content" rows="8"> @if ($edit) {!! $patner->content !!}@endif</textarea>
                    </div>
                    <div class="input-group mb-3">
                        ...
                        <input type="email" class="form-control" id="project-title-input" name="email" @if($edit)
                            value="{{ $patner->email }}" @endif @if ($edit) readonly @endif />
                    </div>
                    <div class="input-group mb-3">
                        <label class="input-group-text" for="project-title-input">Contact</label>
                        <input type="text" class="form-control" id="project-title-input" name="contact" @if($edit)
                            value="{{ $patner->contact }}" @endif @if ($edit) readonly @endif />
                    </div>
                    <div class="input-group mb-3">
                        <label class="input-group-text" for="project-title-input">Site web</label>
                        <input type="text" class="form-control" id="project-title-input" name="website" @if($edit)
                            value="{{ $patner->website }}" @endif @if ($edit) readonly @endif />
                    </div>
                </div>
                <!-- end card body -->
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div>
                        <input type="file" id="patner" name="image_file" accept="image/*" class="filepond" data-show-loader="true"
                            data-allowed-file-extensions="jpg jpeg png gif svg" data-max-file-size="20MB" />
                    </div>
                    @if ($edit)
                        <small>Acienne image</small>
                        <div class="col-4">
                            <a><img src="/public{{ Storage::url("{$patner->image}") }}" alt="{{$patner->title}}" style="width:100%;">
                            </a>
                        </div>
                    @endif
                </div>
...
@if($edit)
    <script>
        var patner={!! json_encode($patnerImage) !!};
    </script>
    <script src="{{ URL::asset('assets/js/pages/patner_form_edit.js') }}"></script>
@else
